...
async def list_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("catalogs/item_types/list.html", {"request": request, "rows": rows})
...
async def detail(row_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("catalogs/item_types/detail.html", {"request": request, "row": row})
