...
async def list_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "catalogs/sizes/list.html", {"request": request, "rows": rows}
    )
...
async def create(
    ...
    return RedirectResponse("/catalogs/sizes/create", status_code=status.HTTP_303_SEE_OTHER)
...
async def detail(row_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "catalogs/sizes/detail.html", {"request": request, "row": row}
    )
