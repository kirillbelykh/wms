...
async def list_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("catalogs/cells/list.html", {"request": request, "rows": rows})
...
async def create(
    ...
    db.commit()
    db.refresh(cell)
    # Опционально привязка товара
    if item_id:
        item = db.query(Item).filter(Item.id == item_id).first()
        if item:
            item.cell_id = cell.id
            db.commit()
    return RedirectResponse(f"/catalogs/cells/{cell.id}/", status_code=status.HTTP_303_SEE_OTHER)
...
async def detail(row_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("catalogs/cells/edit.html", ctx)
...
async def update(
    ...
    db.commit()
    db.refresh(row)
    return RedirectResponse(f"/catalogs/cells/{row_id}/", status_code=status.HTTP_303_SEE_OTHER)
