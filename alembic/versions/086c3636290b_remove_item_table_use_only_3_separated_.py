...
def upgrade() -> None:
    ...
    # === ШАГ 2.5: Добавляем autoincrement для id в supplies, consumables, productions ===
    for table in ['supplies', 'consumables', 'productions']:
        ...
        # Синхронизируем sequence с существующими данными
        op.execute(f"SELECT setval('{seq_name}', COALESCE((SELECT MAX(id) + 1 FROM {table}), 1), false)")
    ...
    # --- consumables ---
    op.add_column('consumables', sa.Column('name', sa.String(), nullable=True))
    op.execute("UPDATE consumables SET name = 'Consumable #' || COALESCE(id::text, 'unknown') WHERE name IS NULL")
    op.alter_column('consumables', 'name', nullable=False)
    ...
    # --- productions ---
    op.add_column('productions', sa.Column('name', sa.String(), nullable=True))
    op.execute("UPDATE productions SET name = 'Production #' || COALESCE(id::text, 'unknown') WHERE name IS NULL")
    op.alter_column('productions', 'name', nullable=False)
    ...
    op.execute("UPDATE supplies SET name = 'Supply #' || COALESCE(id::text, 'unknown') WHERE name IS NULL")
...
def downgrade() -> None:
    ...
    # === Удаляем autoincrement sequences ===
    for table in ['supplies', 'consumables', 'productions']:
        seq_name = f"{table}_id_seq"
